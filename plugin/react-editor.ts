...
export const ReactEditor: ReactEditorInterface = {
  ...
  hasDOMNode: (editor, target, options = {}) => {
    ...
    try {
      targetEl =
        (isDOMElement(target) ? target : target.parentElement) as HTMLElement;
    } catch (err) {
      if (
  ...
  isTargetInsideNonReadonlyVoid: (editor, target) => {
    ...
    const slateNode = ReactEditor.hasTarget(editor, target) &&
  ...
  toDOMRange: (editor, range) => {
    ...
    const endEl =
      // @ts-ignore MIGRATION
      (isDOMElement(endNode) ? endNode : endNode.parentElement) as HTMLElement;
